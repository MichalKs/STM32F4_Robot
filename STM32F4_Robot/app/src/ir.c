...
/**
 * @brief Check if any IR frames arrived.
 * @param c Received command
 * @param a Received address
 * @param t Received toggle
 * @retval 1 Frame received
 * @retval 0 No frames
 */
uint8_t IR_Event(uint8_t* c, uint8_t* a, uint8_t* t) {

  // for now just get last frame
  if (rxCount) {
    rxCount = 0;
    *c = command;
    *a = address;
